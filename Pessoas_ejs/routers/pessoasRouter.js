...
// Quando uma solicitação GET é feita para "/pessoas", o método readList() do controlador de vagas de emprego é chamado
router.get("/api/pessoas", pessoasController.apiReadList);

// Define uma rota GET para ler uma vaga de emprego específica por ID
// Quando uma solicitação GET é feita para "/pessoas/:id", o método read() do controlador de vagas de emprego é chamado,
// onde ":id" é um parâmetro na URL que representa o ID da vaga de emprego
router.get("/api/pessoas/:id", pessoasController.apiRead);

// Define uma rota POST para criar uma nova vaga de emprego
// Quando uma solicitação POST é feita para "/pessoas", o método create() do controlador de vagas de emprego é chamado
router.post("/api/pessoas", pessoasController.apiCreate);

// Define uma rota PUT para atualizar uma vaga de emprego existente por ID
// Quando uma solicitação PUT é feita para "/pessoas/:id", o método update() do controlador de vagas de emprego é chamado,
// onde ":id" é um parâmetro na URL que representa o ID da vaga de emprego
router.put("/api/pessoas/:id", pessoasController.apiUpdate);

// Define uma rota DELETE para excluir uma vaga de emprego existente por ID
// Quando uma solicitação DELETE é feita para "/pessoas/:id", o método delete() do controlador de vagas de emprego é chamado,
// onde ":id" é um parâmetro na URL que representa o ID da vaga de emprego
router.delete("/api/pessoas/:id", pessoasController.apiDelete);
...
router.get("/pessoas/create", pessoasController.viewCreate);

// Define uma rota GET para listar todas as vagas de emprego
router.get("/pessoas", pessoasController.viewReadList);

// Define uma rota GET para visualizar o formulário de atualização de uma vaga de emprego existente
router.get("/pessoas/update/:id", pessoasController.viewUpdate);

// Define uma rota GET para acessar a página inicial
router.get("/", pessoasController.viewHomePage);

// Define uma rota POST para ler uma vaga de emprego com base no que foi digitado na pesquisa
router.post("/pessoas.search", pessoasController.search);
...
router.post("/pessoas/:id", pessoasController.update);

// Define uma rota GET para realizar a exclusão de uma vaga de emprego existente
router.get("/pessoas/delete/:id", pessoasController.delete);
